...
function [error_message] = agk_make_PDT_ss_pspml_pic_0115(cur_sub,cur_tmpl,aggr, ...
...
% try
    ...
    if tworuns
        ...
        % length of each run
        length_run = 101;    % fixed for VPPG0115
        num_run = 2;
    else
    ...
    for ii = 1 : length(P.cur.choice)*2
        % two run correction
        if ii <= length_run
            % first run
            nn = 1;
